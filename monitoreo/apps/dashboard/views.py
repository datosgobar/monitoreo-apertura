...
def indicators_csv(_request, node_id=None, indexing=False):
    if node_id is None:
        ...
        queryset = IndicadorFederador.objects.filter(indicador_tipo__series_federadores=True, jurisdiccion_id=node_id)
    else:
        queryset = Indicador.objects.filter(indicador_tipo__series_nodos=True, jurisdiccion_id=node_id)
...
def csv_zip_response(filename, path):
    if not os.path.exists(path):
        return HttpResponseBadRequest("No hay un archivo generado con ese nombre.")
